 ...
 */
    ...
    (() => {
        ...
        class SelectConstructor {
            ...
            setOptionsPosition(selectItem) {
                ...
                if (!selectItem.classList.contains(this.selectClasses.classSelectOpen)) {
                    ...
                    const selectOptionsWidth = selectOptions.offsetWidth || (selectItemScroll ? selectItemScroll.offsetWidth : 0);
                    if (selectOptionsWidth) selectItem.style.minWidth = `${selectOptionsWidth}px`;
        ...
        const inputMasksPhone = document.querySelectorAll('input[type="tel"]');
        if (inputMasks.length) {
            modules_flsModules.inputmask = Inputmask().mask(inputMasks);
            modules_flsModules.inputmask = Inputmask("+\\ 7 (999) 999-99-99").mask(inputMasksPhone);
            console.log(inputMasksPhone);
        }
        ...
        function initSliders() {
            if (document.querySelector(".product__slider-main")) {
                const thumbsSwiper = new Swiper(".product__slider-small", {
                    modules: [ Navigation ],
                    slidesPerView: 4,
                    spaceBetween: 10,
                    speed: 600,
                ...
                const mainSwiper = new Swiper(".product__slider-main", {
                    ...
                    navigation: {
                        prevEl: ".product__slider-main-prev",
                        nextEl: ".product__slider-main-next"
                    }
                });
            ...
            if (document.querySelector(".project-popup__slider-main")) {
                let projectMainSwiper = null;
                let projectThumbsSwiper = null;
                function initProjectSlider(images) {
                    if (projectMainSwiper) projectMainSwiper.destroy(true, true);
                    if (projectThumbsSwiper) projectThumbsSwiper.destroy(true, true);
                    const mainWrapper = document.querySelector(".project-popup__wrapper-main");
                    const thumbsWrapper = document.querySelector(".project-popup__wrapper-thumbs");
                    mainWrapper.innerHTML = "";
                    thumbsWrapper.innerHTML = "";
                    images.forEach((img, index) => {
                        const mainSlide = document.createElement("div");
                        mainSlide.className = "project-popup__slide-main swiper-slide";
                        mainSlide.innerHTML = `<img src="${img.big}" alt="Проект ${index + 1}">`;
                        mainWrapper.appendChild(mainSlide);
                        const thumbSlide = document.createElement("div");
                        thumbSlide.className = "project-popup__slide-thumb swiper-slide";
                        thumbSlide.innerHTML = `<img src="${img.small}" alt="Проект ${index + 1}">`;
                        thumbsWrapper.appendChild(thumbSlide);
                    });
                    projectThumbsSwiper = new Swiper(".project-popup__slider-thumbs", {
                        modules: [ Navigation ],
                        slidesPerView: 3,
                        spaceBetween: 20,
                        speed: 600,
                    ...
                    projectMainSwiper = new Swiper(".project-popup__slider-main", {
                        modules: [ Thumb, Navigation ],
                        observer: true,
                        observeParents: true,
                        slidesPerView: 1,
                        ...
                        navigation: {
                            prevEl: ".project-popup__slider-prev",
                            nextEl: ".project-popup__slider-next"
                        }
                    });
                    projectMainSwiper.on("slideChange", () => {
                        const idx = projectMainSwiper.realIndex;
                        projectThumbsSwiper.slideTo(idx, 300);
                    });
                }
                window.initProjectSlider = initProjectSlider;
            }
        }
        ...
        document.addEventListener("DOMContentLoaded", () => {
            const catalogBtn = document.querySelector(".menu__link._catalog-btn");
            const submenu = document.querySelector(".menu__list-2lvl");
            const body = document.body;
            if (!catalogBtn || !submenu) return;
            catalogBtn.addEventListener("click", event => {
                event.preventDefault();
                const isOpened = submenu.classList.toggle("_active");
                body.classList.toggle("_submenu-opened", isOpened);
            });
            document.addEventListener("click", event => {
                if (submenu.classList.contains("_active") && !submenu.contains(event.target) && !catalogBtn.contains(event.target)) {
                    submenu.classList.remove("_active");
                    body.classList.remove("_submenu-opened");
                }
            });
        });
        document.addEventListener("click", event => {
            const infoName = event.target.closest(".catalog-card__info-name");
            if (infoName) {
                const catalogInfo = infoName.closest(".catalog-card__info");
                if (catalogInfo) catalogInfo.classList.toggle("_active");
            }
        });
        const filterMobile = document.querySelector(".catalog__sidebar");
        document.addEventListener("click", event => {
            const filterBtn = event.target.closest(".catalog__filter-btn");
            if (filterBtn) if (filterMobile) filterMobile.classList.toggle("_active");
        });
        document.addEventListener("DOMContentLoaded", () => {
            const projectCards = document.querySelectorAll(".project-card");
            const projectPopup = document.querySelector("#project-popup");
            const closeBtn = projectPopup?.querySelector("[data-close]");
            const body = document.body;
            if (!projectCards.length || !projectPopup) return;
            function openProjectPopup(card) {
                const imagesJson = card.getAttribute("data-images");
                if (!imagesJson) {
                    console.error("Не найдены изображения для проекта");
                    return;
                }
                let images = [];
                try {
                ...
                if (window.initProjectSlider) window.initProjectSlider(images);
                projectPopup.classList.add("popup_show");
                body.classList.add("popup-show");
                body.classList.add("lock");
                document.documentElement.classList.add("lock");
            }
            function closeProjectPopup() {
                projectPopup.classList.remove("popup_show");
            ...
            projectPopup.addEventListener("click", e => {
                if (e.target === projectPopup || e.target.classList.contains("popup__wrapper")) closeProjectPopup();
            });
            document.addEventListener("keydown", e => {
                if (e.key === "Escape" && projectPopup.classList.contains("popup_show")) closeProjectPopup();
            });
        });
        window["FLS"] = true;
        menuInit();
